...
- **Les campagnes figées** : `builds/…/bp3`, chacune nommée par son condensat md5. **Ce sont
...
2. ⚠️ **Le numéro de version seul n'est PAS un condensat** (constat #65) : c'est un `#define`
   incrémenté à la main, et l'horodatage vient d'une seule unité de compilation (deux binaires
   distincts peuvent l'afficher identique). **Le seul condensat fiable du contenu d'un binaire est son md5** :
...
⛔ **`./bp3` À LA RACINE N'EST PAS UNE RÉFÉRENCE, ET SON CONDENSAT NE SE COMPARE À RIEN.** Le
...
16:50:13)`. Deux constructions complètes des mêmes sources rendent donc deux condensats md5, alors
...
mesure de référence se prend sur une campagne figée de `builds/`, nommée avec son condensat md5.
